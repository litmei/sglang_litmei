...
The GLM (General Language Model) series is an open-source bilingual large language model family jointly developed by the KEG Laboratory of Tsinghua University and Zhipu AI. This series of models has performed outstandingly in the field of Chinese NLP with its unique unified pre-training framework and bilingual capabilities. [GLM-4.7](https://huggingface.co/zai-org/GLM-4.7) adopts the DeepSeek-V3/V3.2 architecture, including the sparse attention (DSA) and multi-token prediction (MTP). Ascend supports GLM-4.7 with 0Day based on the SGLang inference framework, achieving low-code seamless enablement and compatibility with the mainstream distributed parallel capabilities within the current SGLang framework. We welcome developers to download and experience it.

## Environment Preparation

### Model Weight

- `GLM-4.7.0`(BF16 version): [Download model weight](https://www.modelscope.cn/models/ZhipuAI/GLM-4.7).
- `GLM-4.7.0-w8a8`(Quantized version without mtp): [Download model weight](https://modelers.cn/models/Eco-Tech/GLM-4.7-w8a8).
...
- Quantized model `glm4.7_w8a8` can be deployed on 1 Atlas 800 A3 (64G × 16) .
...
- `GLM-4.7-bf16`: require at least 2 Atlas 800 A3 (64G × 16).
...
python3 -m sglang.launch_server \
        ...
        --attention-backend ascend \
        --device npu \
        --tp-size 16 --dp-size 4 --enable-dp-attention \
        --nnodes 1 --node-rank 0 \
        --chunked-prefill-size 32768 --max-prefill-tokens 520000 \
        --trust-remote-code \
        --host 127.0.0.1 \
        --mem-fraction-static 0.8 \
        ...
        --port 8000 \
        --served-model-name glm-4.7 \
        --cuda-graph-bs 8 16 32 40 \
        --quantization modelslim \
        --moe-a2a-backend deepep --deepep-mode auto \
        --load-balance-method round_robin --enable-dp-lm-head \
        --speculative-draft-model-quantization unquant \
        --speculative-algorithm NEXTH \
        --speculative-num-steps 3 \
        --speculative-eagle-topk 1 \
        --speculative-num-tokens 4 \
```

### 128kk/1k case Deployment
```shell
# high performance cpu
echo performance | tee /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor
sysctl -w vm.swappiness=0
...
python3 -m sglang.launch_server \
        ...
        --attention-backend ascend \
        --device npu \
        --tp-size 16 --dp-size 2 --enable-dp-attention \
        --nnodes 1 --node-rank 0 \
        --chunked-prefill-size 32768 --max-prefill-tokens 520000 \
        --trust-remote-code \
        --host 127.0.0.1 \
