...
class AscendAttnBackend(AttentionBackend):
    ...
    def __init__(self, model_runner: ModelRunner, speculative_step_id: int = 0):
        ...
        self.needs_cpu_seq_lens = not (
            is_deepseek_dsa(model_runner.model_config.hf_config) or self.is_hybrid_swa
        )
    ...
    def init_forward_metadata(self, forward_batch: ForwardBatch):
        ...
        if self.needs_cpu_seq_lens:
            # Legacy path: the block-table width is sized by max(seq_lens),
            # which reads a host value -- the CPU mirror for verify, or an
            # implicit device->int sync at the slice bound below.
            ...
            # needs_cpu_seq_lens=False (DSA): device-only eager metadata,
            # mirroring the graph-replay branch in _apply_cuda_graph_metadata.
            # The block table is gathered at the full fixed width -- KV reads
            # are bounded by the device-side actual_seq_lengths_kv, never by
            # the table width -- so no host value (seq_lens_max / the CPU
            # mirror) is read and no sync is introduced.
            self.forward_metadata.block_tables = (
                self.req_to_token_pool.req_to_token[
        ...
        if forward_batch.seq_lens_cpu is not None:
            ...
            # needs_cpu_seq_lens=False (DSA) nulls the host mirror on
            # decode-family rounds; the sparse-attention kernels take
            # actual_seq_lengths_kv as a device tensor, so mirror the device
            # lengths instead (same values; speculative offsets are applied
            # below and the consumers below .to(device) it anyway).
    ...
    def _apply_cuda_graph_metadata(
        ...
        if self.needs_cpu_seq_lens:
            # Legacy path: the host mirror sizes the block-table copy, so
            # replay metadata waits on the CPU seq_lens.
            ...
            # needs_cpu_seq_lens=False (DSA): replay metadata must not wait on
            # the host seq_lens mirror. The sparse-attention kernels bound KV
            # reads by the device-side actual_seq_lengths_kv (metadata.seq_lens
            # below), so the block-table copy is padded to a fixed width
            # instead of being sized by max(seq_lens); page ids past a
            # request's length are never dereferenced. req_to_token may be
            # narrower than the captured block-table buffer, hence the min.
            total_pages = min(
    ...
    def forward_mtp(
        ...
        if not self.use_mla:
            ...
            if seq_lens_cpu_int is None:
                ...
                # needs_cpu_seq_lens=False (DSA): the device-side lengths are
                # passed to FIA as a tensor (same form as the BSND decode
                # call), avoiding a per-layer D2H sync that would defeat the
                # opt-out.
...
class AscendAttnMultiStepDraftBackend:
    ...
    def __init__(
        ...
        # Uniform across steps (same model_runner). Surfaced so that
        # decide_needs_cpu_seq_lens' OR over spec-v2 backends and the
        # EagleWorkerV2 D2H gating see the per-step backends' opt-out.
