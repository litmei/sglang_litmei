...
class ServerArgs:
    ...
    def _handle_environment_variables(self):
        ...
        # Spec V2 zero-bubble (SGLANG_SPEC_V2_ZERO_BUBBLE): scheduler
        # subprocesses rebuild their environment, so a bare shell export does
        # not survive the spawn boundary. Translate the opt-in env into the
        # serialized ServerArgs field here: the parent sees the env and sets
        # the field; the child re-runs with the env absent, keeping the
        # serialized value. The spec worker reads the field.
