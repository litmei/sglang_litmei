        ...
        if _is_npu:
            if self.deepep_output_dtype == DeepEPOutputDtype.FP8:
                ...
                if _is_npu_before_atlas_a5:
                    raise RuntimeError(
                        "Ascend NPU before Atlas A5 does not support mxfp8 deepep_dispatcher_output_dtype."
                    )
            elif self.deepep_output_dtype == DeepEPOutputDtype.MXFP4:
                if _is_npu_before_atlas_a5:
                    raise RuntimeError(
                        "Ascend NPU before Atlas A5 does not support mxfp4 deepep_dispatcher_output_dtype."
                    )
        else:
