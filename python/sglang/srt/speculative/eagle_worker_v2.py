...
class EAGLEWorkerV2(BaseSpecWorker):
    ...
    def forward_batch_generation(self, batch: ScheduleBatch, on_publish=None):
        if batch.forward_mode.is_extend() or batch.is_extend_in_batch:
            ...
            batch_output: GenerationBatchResult = self.target_worker.forward_batch_generation(batch)

            # if torch.distributed.get_rank() == 0:
            #     import textwrap
            #     _next_token_ids = batch_output.next_token_ids
            #     _hidden_states = batch_output.logits_output.hidden_states
            #     print(textwrap.dedent(f"""--- EAGLEWorkerV2::forward_batch_generation extend target_worker output ---
            #                                   next_token_ids: {None if _next_token_ids is None else _next_token_ids.tolist()}
            #                                   hidden_states: shape {None if _hidden_states is None else tuple(_hidden_states.shape)}, min {None if _hidden_states is None else _hidden_states.min().item()}, max {None if _hidden_states is None else _hidden_states.max().item()}, mean {None if _hidden_states is None else _hidden_states.mean().item()}
            #                                   """))
            ...
            with (
                ...
                # if torch.distributed.get_rank() == 0:
                #     import textwrap
                #     _topk_p = batch_output.next_draft_input.topk_p
                #     _topk_index = batch_output.next_draft_input.topk_index
                #     _draft_probs = batch_output.next_draft_input.draft_probs
                #     _hidden_states = batch_output.next_draft_input.hidden_states
                #     print(textwrap.dedent(f"""--- EAGLEWorkerV2::forward_batch_generation draft prefill output ---
                #                                   topk_p: {None if _topk_p is None else _topk_p.tolist()}
                #                                   topk_index: shape {None if _topk_index is None else tuple(_topk_index.shape)}, min {None if _topk_index is None else _topk_index.min().item()}, max {None if _topk_index is None else _topk_index.max().item()}, mean {None if _topk_index is None else _topk_index.mean().item()}
                #                                   draft_probs: shape {None if _draft_probs is None else tuple(_draft_probs.shape)}, min {None if _draft_probs is None else _draft_probs.min().item()}, max {None if _draft_probs is None else _draft_probs.max().item()}, mean {None if _draft_probs is None else _draft_probs.mean().item()}
                #                                   hidden_states: shape {None if _hidden_states is None else tuple(_hidden_states.shape)}, min {None if _hidden_states is None else _hidden_states.min().item()}, max {None if _hidden_states is None else _hidden_states.max().item()}, mean {None if _hidden_states is None else _hidden_states.mean().item()}
                #                                   """))
                return batch_output
        else:
            ...
            # if torch.distributed.get_rank() == 0:
            #     import textwrap
            #     _spec_info: EagleDraftInput = batch.spec_info
            #     _topk_p = _spec_info.topk_p
            #     _topk_index = _spec_info.topk_index
            #     _draft_probs = _spec_info.draft_probs
            #     _hidden_states = _spec_info.hidden_states
            #     print(textwrap.dedent(f"""--- EAGLEWorkerV2::forward_batch_generation before draft ---
            #                                   topk_p: {None if _topk_p is None else _topk_p.tolist()}
            #                                   topk_index: shape {None if _topk_index is None else tuple(_topk_index.shape)}, min {None if _topk_index is None else _topk_index.min().item()}, max {None if _topk_index is None else _topk_index.max().item()}, mean {None if _topk_index is None else _topk_index.mean().item()}
            #                                   draft_probs: shape {None if _draft_probs is None else tuple(_draft_probs.shape)}, min {None if _draft_probs is None else _draft_probs.min().item()}, max {None if _draft_probs is None else _draft_probs.max().item()}, mean {None if _draft_probs is None else _draft_probs.mean().item()}
            #                                   hidden_states: shape {None if _hidden_states is None else tuple(_hidden_states.shape)}, min {None if _hidden_states is None else _hidden_states.min().item()}, max {None if _hidden_states is None else _hidden_states.max().item()}, mean {None if _hidden_states is None else _hidden_states.mean().item()}
            ...
            # if torch.distributed.get_rank() == 0:
            #     import textwrap
            #     _spec_info: EagleVerifyInput = verify_input
            #     _draft_token = _spec_info.draft_token
            #     _positions = _spec_info.positions
            #     print(textwrap.dedent(f"""--- EAGLEWorkerV2::forward_batch_generation after draft before verify ---
            #                                   draft_token: {None if _draft_token is None else _draft_token.tolist()}
            #                                   positions: {None if _positions is None else _positions.tolist()}
            #                                   """))
            batch_output: GenerationBatchResult = self.verify(batch)
            # if torch.distributed.get_rank() == 0:
            #     import textwrap
            #     _next_token_ids = batch_output.next_token_ids
            #     _hidden_states = batch_output.logits_output.hidden_states
            #     print(textwrap.dedent(f"""--- EAGLEWorkerV2::forward_batch_generation after verify before draft_extend ---
            #                                   next_token_ids: {None if _next_token_ids is None else _next_token_ids.tolist()}
            #                                   hidden_states: shape {None if _hidden_states is None else tuple(_hidden_states.shape)}, min {None if _hidden_states is None else _hidden_states.min().item()}, max {None if _hidden_states is None else _hidden_states.max().item()}, mean {None if _hidden_states is None else _hidden_states.mean().item()}
            #                                   """))
            ...
            # if torch.distributed.get_rank() == 0:
            #     import textwrap
            #     _next_draft_input = batch_output.next_draft_input
            #     _hidden_states = _next_draft_input.hidden_states
            #     print(textwrap.dedent(f"""--- EAGLEWorkerV2::forward_batch_generation after draft_extend ---
            #                                   hidden_states: shape {None if _hidden_states is None else tuple(_hidden_states.shape)}, min {None if _hidden_states is None else _hidden_states.min().item()}, max {None if _hidden_states is None else _hidden_states.max().item()}, mean {None if _hidden_states is None else _hidden_states.mean().item()}
            #                                   """))
