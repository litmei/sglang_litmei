...
def prepare_for_draft_extend(
    ...
    *,
    ...
    with spec_stage_span("dep_setup"):
        if gpu_only:
            batch.prefix_lens = (batch.seq_lens - front_offset).clamp(min=0).to(
                torch.int32
            )
    ...
    with spec_stage_span("dep_initnew"):
    ...
    if not gpu_only:
        # Pin the CPU mirror: the ascend backend copies seq_lens_cpu to the
        # device in init_forward_metadata, and a pinned source keeps that H2D
        # non-blocking (otherwise the CPU stalls ~30ms waiting for the
        # preceding target/verify GPU work to drain).
    ...
    # The cuda-graph metadata path races on the async pinned H2D (extend lens
    # copies) on NPU: a replay-time copy_ shape mismatch ([bs, N+1] vs [bs, N])
    # appears when the copies are still in flight. Settle them before returning
    # -- only on the graph path (eager tolerates the async copies and would
    # only be slowed down by the sync).
